...
impl Game for SixtySix {
    ...
    fn validate(&self, state: &serde_json::Value, action: &Action) -> Result<(), GameError> {
        ...
        match action.action_type.as_str() {
            ...
            ACTION_DECLARE => {
                let suit =
                    get_int(action, "suit").ok_or(GameError("missing suit".to_string()))?;
                let k = card(suit, 4);
                let q = card(suit, 3);
...
mod tests {
    ...
    fn close_stock_enforces_follow_suit() {
        ...
        if let Some(&off) = st2.hands[follower]
            .iter()
            .find(|&&c| card_suit(c) != ls)
        {
            let result = g.validate(&ns, &action_play(off));
            // Should fail if they also have a same-suit card
            if has_suit(&st2.hands[follower], ls) {
                assert!(result.is_err(), "expected follow-suit requirement when closed");
            }
        }
    }
    ...
    fn last_trick_bonus() {
        ...
        assert!(final_st.scores[1] >= 21, "expected last trick bonus, scores={:?}", final_st.scores);
    }
}
